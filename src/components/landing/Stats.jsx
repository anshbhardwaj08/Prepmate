...
const Stats = () => (
  <section className="relative bg-slate-950 py-20 overflow-hidden">
    ...
    <Container>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-slate-800/50 rounded-2xl border border-slate-800 overflow-hidden">
        {stats.map(([num, label]) => (
          <div key={label} className="bg-[#080F1E] hover:bg-slate-900/80 transition-colors duration-200 text-center py-10 px-6">
            <div className="text-4xl sm:text-5xl font-black tracking-tight bg-gradient-to-br from-indigo-400 to-cyan-400 bg-clip-text text-transparent mb-2">{num}</div>
